...
class ReportManager:
    ...
    def _build_report_tree(self, structure_item):
        report_node = {
            ...
            "reports": [report.to_dict() for report in structure_item.analyzer.reports.values() if report.verbosity <= self.verbosity],
    ...
    def render_text(self, node, level=0):
        indent = "    " * level
        report_str = f"{indent}{node['info']['index']} >> {node['info']['mime_type']} {node['info']['size']}\n"
        ...
        for child in node['children']:
            report_str += self.render_text(child, level + 1)
        return report_str
